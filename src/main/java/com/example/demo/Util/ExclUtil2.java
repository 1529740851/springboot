...
public class ExclUtil2 {
	...
	/**
	 * 读取excl文件返回List<String>
	 * @param line  从第几行读起
	 * @couumn column 从第几列读起
	 * */
	public static List<String> importExcel(String fileName,int line,int column){	
		...
		try{
	        ...
	        for (int i = line; i <= rowNum; i++) {	
	        	...
	        	tempStr = getRowValue(sheet.getRow(i),cellsize,column);
	 ...
	 * @return
	 */
	private static String getRowValue(Row row,int cellsize,int column){
		StringBuffer rowValue = new StringBuffer();
		String temp = "";
		for (int i=column;i<cellsize;i++){			
	...
	public   List<Tabletmp>  excl(){
	        ...
	        try {
	           ...
	           // String fileName = "d:\\testExcel.xlsx";
	            List<String> reult = excelReader.importExcel(fileName,1,0);
